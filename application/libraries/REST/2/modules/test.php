...
class TestAPI extends PrototypeAPI
{
	...
	{
		$this->CI->load->model('Property');
		$json = '{"id":"1","number":"1311","route":"Chisholm Trail Rd","subpremise":"303","locality":"Round Rock","admin_level_1":"Texas","admin_level_2":"Williamson","postal_code":"78681","neighborhood":null,"latitude":null,"longitude":null,"info":{"foundation":"slab","reinforcement":"post-tension cables","roof_framing":"factory-built trusses"},"assets":{},"notes":[],"date_added":"1230811200","date_updated":"1343822400"}';		
		$property = new StructProperty($json);
		
		$note = new StructNote();
		
		$note->text 		= 'Note applied to property.';
		$note->user->name	= 'Matthew Solum';
		$note->user->id 	= 1;
		
		$property->notes[] = $note;
		
		$this->CI->Property->commit($property);
