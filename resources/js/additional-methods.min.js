...
jQuery.validator.addMethod("USD", function(value, element){
	return this.optional(element) || /^\$[0-9]+\.[0-9]{2}$/.test(value);
}, "Please enter value as decimal precided by a dollar sign.");

jQuery.validator.addMethod("AtLeastOne", function(value, element){
	var name = $(element).attr('name');

	$.fn.MSDebug("At least one: " + name);
	...
	return false;
}, 'There should be at least one element filled out.');

//Requires all an elements immediate neighbors to also be valid
jQuery.validator.addMethod("SiblingsValid", function(value, element){
	$(element).siblings('input, select').each(function(index, el){
		if(!$(el).valid()) {
			return false;
		}
	});
	return true;
}, 'Immediate neighbors must also be valid.');

jQuery.validator.addMethod("JobItemFilled", function(value, element){
	var prev = $(element).siblings('.job_item').eq(0);
...
function jb_item_filled(element) {
	var prev = $(element).siblings('.job_item').eq(0);
